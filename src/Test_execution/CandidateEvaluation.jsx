            ...
            Promise.all(
                sections.map(sectionId =>
                    Promise.all([
                        ...
                        fetch(`http://127.0.0.1:8000/test-execution/get-answers/?session_id=${sessionId}&section_id=${sectionId}`)
                            .then(res => {
                                if (!res.ok) throw new Error(`Failed to fetch answers for section ${sectionId}`);
                                return res.json();
                            }),
                    ])
                )
